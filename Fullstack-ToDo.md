...
### 7) Update ToDo Fields
- If any of the following fields are defined in the request body, this route should find the todo in the database whose id matches the value of the id param and update these field values:
		- title
		- description
		- isComplete
		- priority
...
- Create a todo's router
- Create the following API routes in the todo's router:
	- /todo/all route that fetches all todo's
	- /todo/create route that creates a new todo
	- /todo/update route that updates a todo
	- /todo/delete route that deletes a todo
...
- Refactor the ToDo Card so that 
- Add client-side validation for creating and updating a todo
- Add server-side validation for creating and updating a todo
- Refactor to have todos under a category
